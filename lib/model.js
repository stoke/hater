...
factoryPrototype.sync = function() {
  ...
  if(this._relationships.hasOne.length) {
    this._relationships.hasOne.map(function(modelName) {
      var child = hater._models[modelName],
          mixin = {},
          parin = {};
      
      mixin[self._name + 'Id'] = Types.Integer; 
      child.addProperties(mixin);

      parin[child._name + 'Id'] = Types.Integer;
      self.addProperties(parin); 
    });
  }

  new Query()
    .createTable(this._table, types.builder(this._schema))
    .exec(function(e, r) {
      return !e ? 
        em.emit('success') :
        em.emit('error', e);
