...
factoryPrototype.save = function(fn) {
  ...
  query.exec(function(e, v) {
    ...
    self.syncOneToOne(function(err) {
 ...
 * Examples:
 *
 *    Instance.syncOnetoOne()
 *
 * @api private
 */

factoryPrototype.syncOneToOne = function(done) {
  var self = this;
  if(hater._relationships.oneToOne[this._name].length) {
    var rels = hater._relationships.oneToOne[this._name],
        len  = rels.length;
    ...
    (function iterator(keys) {
      ...
      var child = self.get(inflection.pluralize(childName));
      
      if(child) {
        child.set(self._name + 'Id', self.get('id'));
        child.save(function(e, instance) {
          if(e)
            return done(e);
          
          self.set(inflection.pluralize(childName), instance);
          iterator(keys);
        });
      }
      else {
