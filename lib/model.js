  ...
  query.exec(function(e, v) {
    if (v) {
      v = v.map(function(x) {
        ...
        if(options.fetch) {
          var len = options.fetch.length;

          options.fetch.forEach(function(elem) {
            var toFetch = elem.split('.');
            
            if(toFetch.length === 1) {
              var sing  = toFetch.shift(),
                  name  = inflection.singularize(sing), 
                  Child = hater._models[name];
              
              if(hater._relationships.oneToOne[instance._name].indexOf(name)) {
                var where = {};
                where[instance._name + 'Id'] = instance.get('id');
                Child.find({ where: where }, function(e, children) {
                  instance.set(sing, children);
                  em.emit('done', e, v);
                });
              }
            }
