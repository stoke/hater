...
factoryPrototype.save = function(fn) {
  var self = this, query, u = {};

  var em = new Emitter();

  query = new Query();

  Object.keys(this._properties).forEach(function(x) {
    if (~Object.keys(this._schema).indexOf(x))
      u[x] = this._properties[x];
  });
  
  if (this.get('id')) {
    query.update(this._table, u).where({id: this.get('id')});
  } else {
    query.insert(this._table, u);
  }
