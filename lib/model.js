    ...
    function(elem, cbl) {
      ...
      async.forEach(childNames,
        function(childName, done) {
          var clause = { where: {}, limit: options.limit },
              Child;
         ...
         if(
            ...
            if(manyToMany && ~manyToMany.indexOf(inflection.singularize(childName))) {
              ...
              var join = {}, where = {};
              join[inflection.pluralize(Child._name) + '.id'] = inflection.pluralize(Table._name) + '.' + Child._name + '_id';
              where[inflection.pluralize(Table._name) + '.' + instance._name + '_id'] = instance.get('id');
              ...
              if(options.limit && (options.limit[Child._name] || options.limit[Child._table])) {
                var limit;
                if(options.limit[Child._name]) {
                  limit = options.limit[Child._name];
                  delete options.limit[Child._name];
                }
                else {
                  limit = options.limit[Child._table];
                  delete options.limit[Child._table];
                }
                q.limit.apply(q, Array.isArray(limit) ?
                  limit.map(function(v) { return parseInt(v, 10); }) :
                  [parseInt(limit, 10)] );
              }

              q.exec(function(e, res) {
                  var many = []; 
              
                if(!res || !res.length) return done(null);
                  ...
                  async.forEach(res,
                    function(row, cbl) {
                      ...
                      Child.findOne(clause, function(e, res) {
                        if(e) return cbl(e);
  
                        if(res) many.push(res);
