 ...
 * Examples:
 *
 *    builder.delete('table', { where: { id: 1 } }, function(e, res) { ... });
 *
 * @param {String} table
 * @param {Object} where
 * @param {Function} callback
...
builder.delete = function(table, where, callback) {
  client.query(
    'DELETE FROM ' + table + buildClause(where), callback
  );
};
...
/**
 * Parses `options` object to build WHERE clause & more
 *
 * Examples:
 *    
 *    buildClause({ where: { id: 1 } });
 *
 * @param {Object} obj
 * @api private
 */

function buildClause(obj, number) {
  number = number || 1;
  var str = '';
